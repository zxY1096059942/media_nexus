...
public class MemberService {
    ...
    public int insert(Member member) {
        ...
        QueryWrapper<Member> qw = new QueryWrapper<>();
        qw.eq("phone_encrypted", AesCryptoUtils.encrypt(aesKey, member.getPhoneHidden()));
        Member validateMember = memberMapper.selectOne(qw);
        ...
        member.setPhoneEncrypted(AesCryptoUtils.encrypt(aesKey, member.getPhoneHidden()));
        member.setPhoneHidden(PhoneUtils.hidePhone(member.getPhoneHidden()));


        member.setPassword(SecurityUtils.encryptPassword(member.getPassword()));
       // member.setNickname("用户" + member.getPhoneHidden().substring(7,11));
    ...
    public Integer changeFirstRunTime(MemberVO dto) {
        UpdateWrapper<Member> wrapper = new UpdateWrapper<>();
        wrapper.eq("id", dto.getId());
        wrapper.set("first_run_time", dto.getFirstRunTime());
        return memberMapper.update(null, wrapper);
    }
